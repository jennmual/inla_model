...
fig1 <- ggplot(coefs_forest,
  ...
  labs(x     = "Cambio en log(FOI)",
       ...
       subtitle = "FOI tiempo-variante — modelo BESAG, univariable vs multivariable") +
...
fig2 <- ggplot(coefs_comparacion,
  ...
  labs(x        = "Cambio en log(FOI)",
       ...
       subtitle = "Modelo B multivariable — efecto espacial BESAG") +
  theme_minimal(base_size = 13) +
  theme(legend.position = "bottom")
...
fig3 <- ggplot(mun_mapa) +
  ...
  labs(title    = "Riesgo relativo espacial de FOI — Colombia",
       subtitle = "Efecto espacial estructurado BESAG, Modelo B") +
